...
<body class="m-0 p-0 bg-[#e3dac9] font-sans min-h-screen text-gray-800">
    ...
    <main class="max-w-[1100px] mx-auto pt-10 pb-12 px-6">
        ...
        @if(session('success'))
            <div class="mb-6 p-4 bg-green-100 text-green-700 rounded-xl text-center font-bold">{{ session('success') }}</div>
        @endif
        @if($errors->any())
            <div class="mb-6 p-4 bg-red-100 text-red-700 rounded-xl text-center font-bold">{{ $errors->first() }}</div>
        @endif
            ...
            <div class="bg-[#f5eedc] rounded-[24px] p-6 shadow-sm flex flex-col justify-between h-full">
                ...
                <div class="flex items-center gap-5 mb-6">
                    <div class="w-24 h-24 rounded-full overflow-hidden bg-white shadow-inner flex-shrink-0">
                        <img 
                            src="{{ !empty($menu['image']) ? env('CAFIER_API_STORAGE_URL') . '/' . $menu['image'] : 'https://images.unsplash.com/photo-1572442388796-11668a67e53d?q=80&w=200&auto=format&fit=crop' }}" 
                            alt="{{ $menu['name'] }}" 
                ...
                <div class="flex items-center gap-3 mt-auto">
                    <button 
                        onclick="openEditModal('{{ $menu['id'] }}', '{{ addslashes($menu['name']) }}', '{{ $menu['price'] }}', '{{ addslashes($menu['description'] ?? '') }}', '{{ $menu['category_id'] }}')"
                        class="flex-1 bg-[#ffcc00] hover:bg-[#e6b800] text-white text-[15px] font-bold py-3 rounded-full transition-colors"
                    >
                        Ubah Menu
                    </button>
                    
                    <form action="{{ url('/daftar-menu/delete/' . $menu['id']) }}" method="POST" class="m-0 p-0 flex" onsubmit="return confirm('Yakin mau hapus menu {{ addslashes($menu['name']) }}?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-[46px] h-[46px] bg-[#ff4b4b] hover:bg-[#e64343] text-white rounded-full flex items-center justify-center transition-colors flex-shrink-0 cursor-pointer border-none">
   ...
   <script>
        // 🚨 SCRIPT LOGIKA JAVASCRIPT MODAL
        function openAddModal() {
            ...
            document.getElementById('method_spoof').value = "POST"; // Method Add pake POST
            ...
            // 🚨 ACTION BUAT TAMBAH MENU (Pake url() biar dinamis ngikutin env APP_URL)
        ...
        function openEditModal(id, name, price, desc, categoryId) {
            ...
            document.getElementById('method_spoof').value = "PUT"; // Method Edit pake PUT
            ...
            // 🚨 ACTION BUAT UBAH MENU (Ditambah ID di belakangnya)
            form.action = "{{ url('/daftar-menu/update') }}/" + id;
