...
<body class="m-0 p-0 bg-[#e3dac9] font-sans h-screen w-full flex text-gray-800">
    ...
    <div class="flex-1 px-12 lg:px-[100px] flex flex-col justify-center">
        ...
        @if($errors->any())
            <div class="max-w-[550px] mx-auto w-full mb-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-xl text-center font-semibold">
                {{ $errors->first() }}
            </div>
        @endif
        
        <form action="{{ route('login.process') }}" method="POST" autocomplete="off" class="max-w-[550px] mx-auto w-full">
            @csrf 
            
            <div class="mb-[20px]">
                <label for="login" class="block text-[18px] mb-2 font-semibold text-[#3a2215]">Username</label>
                <input type="text" id="login" name="login" value="{{ old('username') }}" required autocomplete="new-username"
