...
// ================== KOMPONEN UTAMA ==================
export default function DashboardBaristaScreen() {
  ...
  return (
    // Pake ScrollView biar kalo kontennya panjang bisa di-scroll
    // Background color udah diurusin sama _layout.tsx, jadi di sini transparent aja / ngikut
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      
      {/* SECTION 1: KARTU SUMMARY */}
              ...
              <View style={styles.dropdownMenu}>
                {['Harian', 'Mingguan', 'Bulanan'].map((option) => (
                  <TouchableOpacity 
                    ...
                    onPress={() => { setFilter(option); setIsDropdownOpen(false); setActiveBarIndex(null); }}
                  >
        ...
        <View style={styles.barChartContainer}>
          {chartData.map((data, index) => (
            <View key={index} style={styles.barWrapper}>
              ...
              <TouchableOpacity 
                style={styles.barsGroup} 
                activeOpacity={0.8} 
                onPress={() => setActiveBarIndex(activeBarIndex === index ? null : index)}
              >
